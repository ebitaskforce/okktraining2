...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 flex flex-col transition-colors">
      ...
      {/* Hero Banner with Custom Background Picture & Editable Wordings */}
      <section className="relative overflow-hidden bg-slate-900 text-white py-20 lg:py-28">
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-30 mix-blend-overlay transition-all duration-500"
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
          <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold bg-blue-500/20 border border-blue-400/30 text-blue-300 mb-6 backdrop-blur-md">
            <Sparkles className="w-3.5 h-3.5" />
            {orgSettings.hero_badge_text || 'Official Government Training & Capacity Building Platform'}
          </span>

          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight max-w-4xl mb-6">
            {orgSettings.hero_title || 'Empowering Excellence Through Specialized Training'}
          </h1>

          <p className="text-base sm:text-lg text-slate-300 max-w-3xl mb-10 leading-relaxed">
            {orgSettings.hero_subtitle || `Welcome to ${orgSettings.organization_name}. Browse upcoming professional development courses, register for morning or afternoon slots, and track your booking approvals seamlessly.`}
          </p>
      ...
      {/* Key System Features */}
      <section className="bg-slate-100 dark:bg-slate-800/50 py-16 border-y border-slate-200 dark:border-slate-800 my-12">
